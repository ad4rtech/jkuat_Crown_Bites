...
import { ChevronLeft, Search, ChevronRight, HeadphonesIcon, Settings, UtensilsCrossed, Receipt, Package, Users } from 'lucide-react-native';
import { useRouter, Stack } from 'expo-router';
import Animated, { FadeInDown } from 'react-native-reanimated';

export default function ManagerHelpScreen() {
  ...
  const ISSUES = [
    {
      ...
      desc: 'How to mark ingredients or menu items out of stock across the entire ROKMS network.',
      icon: <UtensilsCrossed size={24} color="#db8221" />,
    },
    {
      id: 'void-auth',
      title: 'Void Authorizations',
      desc: 'Approving requested voids, refunds, and discounts for the Cashier and Waitstaff.',
      icon: <Receipt size={24} color="#db8221" />,
    },
    {
      id: 'inventory-audit',
      title: 'Inventory Auditing',
      desc: 'Performing end-of-week inventory variance checks and supplier requisition.',
      icon: <Package size={24} color="#db8221" />,
    },
    {
      id: 'staff-onboarding',
      title: 'Staff Roles & Onboarding',
      desc: 'Managing access controls, resetting PINs, and creating temporary staff profiles.',
      icon: <Users size={24} color="#db8221" />,
    },
    {
      id: 'eod-reports',
      title: 'End of Day (EOD) Reports',
      desc: 'Generating and verifying the Z-Report and revenue dashboard analytics.',
      icon: <Settings size={24} color="#db8221" />,
    },
  ];
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top + 10, paddingBottom: insets.bottom + 20 }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Manager Help</Text>
        <View style={{ width: 44 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.searchBox}>
          ...
          <TextInput
            ...
            placeholder="Search manager guides..."
          ...
          {ISSUES.filter(issue => issue.title.toLowerCase().includes(searchQuery.toLowerCase()) || issue.desc.toLowerCase().includes(searchQuery.toLowerCase())).map((issue, index) => (
            <Animated.View key={issue.id} entering={FadeInDown.delay(index * 100).duration(400)}>
              <TouchableOpacity 
                ...
                onPress={() => router.push((`/manager-help/${issue.id}` as any))}
                activeOpacity={0.8}
              >
        ...
        <TouchableOpacity style={styles.contactCard} activeOpacity={0.9}>
          ...
          <View style={styles.contactTextContainer}>
            <Text style={styles.contactTitle}>System Administrator</Text>
            <Text style={styles.contactDesc}>Contact IT for app failure</Text>
          </View>
        </TouchableOpacity>
