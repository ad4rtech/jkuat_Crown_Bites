  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top + 10, paddingBottom: insets.bottom + 20 }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Kitchen Alerts {unreadCount > 0 && `(${unreadCount})`}</Text>
      </View>

      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {notifications.length === 0 ? (
          <Text style={{ textAlign: 'center', color: '#8a7465', marginTop: 40 }}>No kitchen alerts to display.</Text>
        ) : (
...
const styles = StyleSheet.create({
  ...
  cardNew: {
    borderLeftWidth: 4,
    borderLeftColor: '#ef4444', // Red left border for urgent kitchen alerts
  },
  newIndicator: {
    position: 'absolute',
    top: 16,
